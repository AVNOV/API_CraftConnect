...
export class ReasonService {
  ...
  async findAll(): Promise<ReasonAppointment[]> {
    return await this.reasonAppointment.find({ relations: ['artisanSkill'] });
  }

  async findOne(id: number): Promise<ReasonAppointment | null> {
    return await this.reasonAppointment.findOne({
      where: { id },
      relations: ['artisanSkill'],
    });
  }
